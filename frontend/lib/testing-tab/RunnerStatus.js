...
import './RunnerStatus.scss'; // Import the stylesheet

const RunnerStatus = ({ editor }) => {
  const {
    runnerConfig = {
      host: 'localhost',
      port: 36227
  ...
  const [ host, setHost ] = useState(runnerConfig.host);
  const [ port, setPort ] = useState(runnerConfig.port);
